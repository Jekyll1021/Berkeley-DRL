            ...
            for i in range(20):
                ...
                while not done:
                    ...
                    if steps >= max_steps:
                        break
                total.append(totalr)
            np.append(obs_data, np.array(observations), axis=0)
            actions = np.array(actions).reshape(-1, act_data.shape[1])
            np.append(act_data, actions, axis=0)
            returns2.append(sum(total)/len(total))
            print(total, sum(total)/len(total))

    print('returns', returns2)
    print('mean return', np.mean(returns2))
    print('std of return', np.std(returns2))

    # train = [1000*x for x in range(0, len(returns))]
    # plt.plot(train, returns, "r", label='behavioral_cloning')
    # plt.plot(train, returns2, "b", label='behavioral_cloning')
    # plt.legend(loc='upper right')
    # plt.xlabel('training steps')
    # plt.ylabel('returns (BC vs DAgger)')
    # pp = PdfPages('q4_2.pdf')
