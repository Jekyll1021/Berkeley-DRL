    ...
    for i in range(num_paths):
        ...
        for i in range(horizon):
            ...
            if done:
                ...
                obs = next_obs
        path['observations'], path['actions'], path['next_observations'], path['rewards'] = np.array(observations), np.array(actions), np.array(next_observations), np.array(rewards)
        paths.append(path)
...
def plot_comparison(env, dyn_model):
    ...
    for _ in range(100):
        ...
        pred.append(dyn_model.predict(np.array([state]), np.array([action])))
...
def train(env, 
    ...
    """ YOUR CODE HERE """
    rollouts = sample(env, random_controller, num_paths_random)
    data = {}
    data['observations'] = np.concatenate([path['observations'] for path in rollouts])
    data['actions'] = np.concatenate([path['actions'] for path in rollouts])
    data['next_observations'] = np.concatenate([path['next_observations'] for path in rollouts])

    random_returns = [path["rewards"].sum() for path in rollouts]
    random_costs = []
    for path in rollouts:
        random_costs.append(path_cost(cheetah_cost_fn, path))

    print(np.mean(random_returns))
    print(np.mean(random_costs))
    ...
    for itr in range(onpol_iters):
        ...
        new_data = sample(env, mpc_controller, num_paths_onpol)
        
        data["observations"] = np.append(data["observations"], np.concatenate([path['observations'] for path in new_data]), axis=0)
        data["actions"] = np.append(data["actions"], np.concatenate([path['actions'] for path in new_data]), axis=0)
        data["next_observations"] = np.append(data["next_observations"], np.concatenate([path['next_observations'] for path in new_data]), axis=0)

        print([path["rewards"] for path in new_data])

        returns = [path["rewards"].sum() for path in new_data]
        costs = []
        for path in new_data:
            costs.append(path_cost(cheetah_cost_fn, path))
